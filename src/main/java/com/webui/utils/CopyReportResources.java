...
public class CopyReportResources {
	...
	public void copyResources() {
		...
		// this.copyFile("src/main/resources/reportResources//logo1.png",
		// "test-output/logo1.png");
	...
	private void copyFile(String sourceRelativePath, String targetRelativePath) {
		FileInputStream fileInputStream = null; // 读取流字节流
		OutputStream imageOutputStream = null; // 写入流字节流
		try {
			...
			while ((startbyte = fileInputStream.read(b)) != -1) {// 读取
				imageOutputStream.write(b, 0, startbyte);// 写入，读多少写入多少，所以用write(b,0,len)
			}
