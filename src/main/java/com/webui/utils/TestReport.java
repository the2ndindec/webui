...
import com.webui.config.Config;
...
public class TestReport implements IReporter {
	...
	public void generateReport(List<XmlSuite> xmlSuites, List<ISuite> suites, String outputDirectory) {
		...
		this.sort(list);
		this.outputResult(list, outputDirectory + "/xx1WebTest_report.html");
	}
	...
	private void outputResult(List<ITestResult> fullResults, String path) {
		...
		try {
			...
			sb.append("<script type=\"text/javascript\">\n")
					...
					.append(" $(document).ready(function(){" + "$(\".passed_button\").click(function(){\n"
			...
			// sb.append("<div id=\"logo\"><img
			// src=\"logo1.png\"></img></div>\n");
			sb.append("<div style=\"clear:both\"><div id=\"time\">\n");
			sb.append("<div style=\"pading-top:20px\">开始时间：" + this.formatDate(fullResults.get(0).getStartMillis())
					+ "</div>\n");
			sb.append("<div>结束时间：" + this.formatDate(fullResults.get(fullResults.size() - 1).getEndMillis())
					+ " </div>\n");
			sb.append("<div>用&nbsp&nbsp&nbsp&nbsp时：" + formateTime(
					fullResults.get(fullResults.size() - 1).getEndMillis() - fullResults.get(0).getStartMillis())
					+ "</div>\n");
			sb.append("</div>\n");
			...
			sb.append("</button>");
			sb.append("</div></td>\n<td><div align=\"center\">");
			sb.append("<button id=\"passed_num\"" + "value=\"" + passArrayList.size() + "\">");
			sb.append(passArrayList.size());
			sb.append("</button>");
			sb.append("</div></td>\n <td><div align=\"center\">");
			sb.append("<button id=\"failed_num\"" + "value=\"" + failedArrayList.size() + "\">");
			sb.append(failedArrayList.size());
			sb.append("</button>");
			sb.append("</div></td>\n<td><div align=\"center\">");
			sb.append("<button id=\"skiped_num\"" + "value=\"" + skipArrayList.size() + "\">");
			...
			for (int i = 0; i < fullResults.size(); i++) {
				...
				String testMethodFullName = fullResults.get(i).getTestClass().getName() + "."
						+ fullResults.get(i).getMethod().getMethodName();
				if (!fullResults.get(i).getMethod().getDescription().isEmpty()) {
					if (fullResults.get(i).getParameters().length > 0) {
						...
						if (parameters[0].toString().contains("院") || parameters[0].toString().contains("所")
								|| parameters[0].toString().contains("医") || parameters[0].toString().contains("病")
								|| parameters[0].toString().contains("治")) {
							sb.append("<td class=\"case_funtcion_td\">" + fullResults.get(i).getTestContext().getName()
									+ "</td>\n");
							sb.append("<td class=\"case_name_td\">" + fullResults.get(i).getMethod().getDescription()
									+ ":" + parameters[0].toString() + "</td>\n");
						} else {
							sb.append("<td class=\"case_funtcion_td\">" + fullResults.get(i).getTestContext().getName()
									+ "</td>\n");
							sb.append("<td class=\"case_name_td\">" + fullResults.get(i).getMethod().getDescription()
					...
					sb.append("<span class=\"full_info\" id=\"fullresult_"
							+ fullResults.get(i).getMethod().getMethodName() + "_info\" style=\"display:none\" " + ">");
					/*
					 * .append("用例基本信息：<br />\n");
					 * .append("method:"+testMethodFullName+"<br />\n")
					 * .append("start_time:"+this.formatDate(fullResults.get(i).
					 * getStartMillis()) +"<br />\n")
					...
					if (fullResults.get(i).getParameters().length > 0) {
						sb.append("<td class=\"case_result_beizhu_td\">" + "输入数据："
								+ fullResults.get(i).getParameters()[0] + "</td>\n");
					} else {
			...
			for (int j = 0; j < skipArrayList.size(); j++) {
				...
				String testMethodFullName = skipArrayList.get(j).getTestClass().getName() + "."
						+ skipArrayList.get(j).getMethod().getMethodName();
				if (!skipArrayList.get(j).getMethod().getDescription().isEmpty()) {
					if (skipArrayList.get(j).getParameters().length > 0) {
						sb.append(
								"<tr id=\"" + testMethodFullName + "." + skipArrayList.get(j).getEndMillis() + "\">\n");
						sb.append("<td class=\"SEQ\">" + (j + 1) + "</td>");
						Object[] parameters = skipArrayList.get(j).getParameters();
						if (parameters[0].toString().contains("院") || parameters[0].toString().contains("所")
								|| parameters[0].toString().contains("医") || parameters[0].toString().contains("病")
								|| parameters[0].toString().contains("治")) {
							sb.append("<td class=\"case_funtcion_td\">"
									+ skipArrayList.get(j).getTestContext().getName() + "</td>\n");
							sb.append("<td class=\"case_name_td\">" + skipArrayList.get(j).getMethod().getDescription()
									+ ":" + parameters[0].toString() + "</td>\n");
						} else {
							sb.append("<td class=\"case_funtcion_td\">"
									+ skipArrayList.get(j).getTestContext().getName() + "</td>\n");
						...
						sb.append("<tr id=\"" + testMethodFullName + "\">\n");
						sb.append("<td class=\"SEQ\">" + (j + 1) + "</td>");
						sb.append("<td class=\"case_funtcion_td\">" + skipArrayList.get(j).getTestContext().getName()
								+ "</td>\n");
						sb.append("<td class=\"case_name_td\">" + skipArrayList.get(j).getMethod().getDescription()
								+ "</td>\n");
					}
					sb.append(" ").append("<td class=\"case_result_info_td\">"
							+ this.getStatus(skipArrayList.get(j).getStatus()) + "</td>\n");
					if (skipArrayList.get(j).getParameters().length > 0) {
						sb.append("<td class=\"case_result_beizhu_td\">" + "输入数据："
								+ skipArrayList.get(j).getParameters()[0] + "</td>\n");
					} else {
						sb.append("<td class=\"case_result_beizhu_td\">" + "输入数据：无" + "</td>\n");
					}
			...
			for (int i = 0; i < passArrayList.size(); i++) {
				...
				String testMethodFullName = passArrayList.get(i).getTestClass().getName() + "."
						+ passArrayList.get(i).getMethod().getMethodName();
				if (!passArrayList.get(i).getMethod().getDescription().isEmpty()) {
					if (passArrayList.get(i).getParameters().length > 0) {
						sb.append(
								"<tr id=\"" + testMethodFullName + "." + passArrayList.get(i).getEndMillis() + "\">\n");
						sb.append("<td class=\"SEQ\">" + (i + 1) + "</td>");
						...
						if (parameters[0].toString().contains("院") || parameters[0].toString().contains("所")
								...
								|| parameters[0].toString().contains("治")) {
							sb.append("<td class=\"case_funtcion_td\">"
									+ passArrayList.get(i).getTestContext().getName() + "</td>\n");
							sb.append("<td class=\"case_name_td\">" + passArrayList.get(i).getMethod().getDescription()
									+ ":" + parameters[0].toString() + "</td>\n");
						} else {
							sb.append("<td class=\"case_funtcion_td\">"
									+ passArrayList.get(i).getTestContext().getName() + "</td>\n");
						...
						sb.append("<tr id=\"" + testMethodFullName + "\">\n");
						sb.append("<td class=\"SEQ\">" + (i + 1) + "</td>");
						sb.append("<td class=\"case_funtcion_td\">" + passArrayList.get(i).getTestContext().getName()
								+ "</td>\n");
						sb.append("<td class=\"case_name_td\">" + passArrayList.get(i).getMethod().getDescription()
								+ "</td>\n");
					}
					sb.append(" ")
							.append("<td class=\"case_result_td\"><button class=\"passed_button\" id=\"passedresult_"
									+ passArrayList.get(i).getMethod().getMethodName() + "\">"
									+ this.getStatus(passArrayList.get(i).getStatus()) + "</button><br /></td>" + "\n")
							.append("<td  class=\"case_result_info_td\">")
							...
							/*
							 * .append("method:"+testMethodFullName+"<br />\n")
							 * .append("start_time:"+this.formatDate(
							 * passArrayList.get(i).getStartMillis()) +
							 * "<br />\n")
							 * .append("time:"+Float.toString(t)+"毫秒"+"<br />\n"
							 * )
							 */
					...
					if (passArrayList.get(i).getParameters().length > 0) {
						sb.append("<td class=\"case_result_beizhu_td\">" + "输入数据："
								+ passArrayList.get(i).getParameters()[0] + "</td>\n");
					} else {
						sb.append("<td class=\"case_result_beizhu_td\">" + "输入数据：无" + "</td>\n");
					}
			...
			for (int j = 0; j < failedArrayList.size(); j++) {
				...
				String testMethodFullName = failedArrayList.get(j).getTestClass().getName() + "."
						+ failedArrayList.get(j).getMethod().getMethodName();
				if (!failedArrayList.get(j).getMethod().getDescription().isEmpty()) {
					...
					if (failedArrayList.get(j).getParameters().length > 0) {
						...
						if (parameters[0].toString().contains("院") || parameters[0].toString().contains("所")
								...
								|| parameters[0].toString().contains("治")) {
							sb.append("<td class=\"case_funtcion_td\">"
									+ failedArrayList.get(j).getTestContext().getName() + "</td>\n");
							sb.append(
									"<td class=\"case_name_td\">" + failedArrayList.get(j).getMethod().getDescription()
											+ ":" + parameters[0].toString() + "</td>\n");
						} else {
							sb.append("<td class=\"case_funtcion_td\">"
									+ failedArrayList.get(j).getTestContext().getName() + "</td>\n");
							sb.append("<td class=\"case_name_td\">"
									+ failedArrayList.get(j).getMethod().getDescription() + "</td>\n");
						}
					} else {
						sb.append("<tr id=\"" + testMethodFullName + "\">\n");
						sb.append("<td class=\"SEQ\">" + (j + 1) + "</td>");
						sb.append("<td class=\"case_funtcion_td\">" + failedArrayList.get(j).getTestContext().getName()
								+ "</td>\n");
						sb.append("<td class=\"case_name_td\">" + failedArrayList.get(j).getMethod().getDescription()
								+ "</td>\n");
					}
					sb.append(" ")
							.append("<td class=\"case_result_td\" ><button class=\"failed_button\" id=\"failedresult_"
									+ failedArrayList.get(j).getMethod().getMethodName() + "\">"
									+ this.getStatus(failedArrayList.get(j).getStatus()) + "</button><br /></td>"
									+ "\n")
							.append("<td  class=\"case_result_info_td\">")
							 ...
							 * .append("method:"+testMethodFullName+"<br />\n")
							 * .append("start_time:"+this.formatDate(
							 * failedArrayList.get(j).getStartMillis() )+
							 * "<br />\n")
							 * .append("time:"+Float.toString(t)+"毫秒"+"<br />\n"
							 * )
							 */
					...
					if (failedArrayList.get(j).getParameters().length > 0) {
						sb.append("<td class=\"case_result_beizhu_td\">" + "输入数据："
								+ failedArrayList.get(j).getParameters()[0] + "</td>\n");
					} else {
						sb.append("<td class=\"case_result_beizhu_td\">" + "输入数据：无" + "</td>\n");
					}
					sb.append("<td class=\"case_time_td\">" + formateTime((long) t) + "</td>\n");
					...
					sb.append("<td colspan=\"3\" style=\"display:none\">").append("</td>\n").append("</tr>\n");
				}
			...
			// sb2.append("<div id=\"logo\"><img
			// src=\"http://218.17.157.250:8848/logo1.png\"></img></div>\n");
			sb2.append("<div style=\"clear:both\"><div id=\"time\">\n");
			sb2.append("<div style=\"pading-top:20px\">开始时间：" + this.formatDate(fullResults.get(0).getStartMillis())
					+ "</div>\n");
			sb2.append("<div>结束时间：" + this.formatDate(fullResults.get(fullResults.size() - 1).getEndMillis())
					+ " </div>\n");
			sb2.append("<div>用&nbsp&nbsp&nbsp&nbsp时：" + formateTime(
					fullResults.get(fullResults.size() - 1).getEndMillis() - fullResults.get(0).getStartMillis())
					+ "</div>\n");
			sb2.append("</div>\n");
			...
			sb2.append("<div style=\"text-align:center\">" + reportTitle + "</div>\n");
			sb2.append("</div>\n");
			sb2.append("</div>\n");
			sb2.append("<div id=\"report_total\">\n");
			sb2.append(
					"<div>(<span style=\"color:green\">绿色字体pass用例</span><span style=\"color:red\">红色字体failed用例</span><span style=\"color:gray\">灰色字体skip用例</span>)<span style=\" margin-left:20px\"><a href=\" "
							+ reportUrl + "\">点击查看完整报表</a></span><span style=\"margin-left:14px\"><a href=\"" + logUrl
							+ "\"></a></span></div>\n");
			sb2.append(
			...
			sb2.append(
					"</div></td>\n<td style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\"><div align=\"center\">");
			sb2.append("<label style=\"color:green\" id=\"passed_num\"" + "value=\"" + passArrayList.size() + "\">");
			sb2.append(passArrayList.size());
			sb2.append("</label>");
			sb2.append(
					"</div></td>\n <td style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\"><div align=\"center\">");
			sb2.append("<label style=\"color:red\" id=\"failed_num\"" + "value=\"" + failedArrayList.size() + "\">");
			sb2.append(failedArrayList.size());
			sb2.append("</label>");
			sb2.append(
					"</div></td>\n<td style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\"><div align=\"center\">");
			sb2.append("<label style=\"color:gray\" id=\"skiped_num\"" + "value=\"" + skipArrayList.size() + "\">");
				...
				if (!failedArrayList.get(j).getMethod().getDescription().isEmpty()) {
					...
					if (failedArrayList.get(j).getParameters().length > 0) {
						sb2.append("<tr  style=\"color:red\" id=\"" + testMethodFullName + "."
								+ failedArrayList.get(j).getEndMillis() + "\">\n");
						sb2.append(
								"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\"  class=\"SEQ\">"
										+ (j + 1) + "</td>");
						Object[] parameters = failedArrayList.get(j).getParameters();
						if (parameters[0].toString().contains("院") || parameters[0].toString().contains("所")
								|| parameters[0].toString().contains("医") || parameters[0].toString().contains("病")
								|| parameters[0].toString().contains("治")) {
							sb2.append(
									"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\"  class=\"case_funtcion_td\">"
											+ failedArrayList.get(j).getTestContext().getName() + "</td>\n");
							sb2.append(
									"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\"  class=\"case_name_td\">"
											+ failedArrayList.get(j).getMethod().getDescription() + ":"
											+ parameters[0].toString() + "</td>\n");
						} else {
							sb2.append(
									"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_funtcion_td\">"
					...
					sb2.append(" ")
							.append("<td class=\"case_result_td\" ><label class=\"failed_label\" id=\"failedresult_"
									+ failedArrayList.get(j).getMethod().getMethodName() + "\">"
									+ this.getStatus(failedArrayList.get(j).getStatus()) + "</label><br /></td>"
									+ "\n");
					if (failedArrayList.get(j).getParameters().length > 0) {
						sb2.append(
								"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_result_beizhu_td\">"
										+ "输入数据：" + failedArrayList.get(j).getParameters()[0] + "</td>\n");
					} else {
						sb2.append(
								"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_result_beizhu_td\">"
										+ "输入数据：无" + "</td>\n");
					...
					sb2.append(
							"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_time_td\">"
									+ formateTime((long) t) + "</td>\n");
					// 点击失败按钮，弹出失败信息

					sb2.append("<td colspan=\"3\" style=\"display:none\">").append("</td>\n").append("</tr>\n");
				}
				...
				if (!passArrayList.get(i).getMethod().getDescription().isEmpty()) {
					if (passArrayList.get(i).getParameters().length > 0) {
						sb2.append("<tr style=\"color:green\" id=\"" + testMethodFullName + "."
								+ passArrayList.get(i).getEndMillis() + "\">\n");
						sb2.append(
								"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"SEQ\">"
										+ (i + failedArrayList.size() + 1) + "</td>");
						Object[] parameters = passArrayList.get(i).getParameters();
						if (parameters[0].toString().contains("院") || parameters[0].toString().contains("所")
								|| parameters[0].toString().contains("医") || parameters[0].toString().contains("病")
								|| parameters[0].toString().contains("治")) {
							...
							sb2.append(
									"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_name_td\">"
											+ passArrayList.get(i).getMethod().getDescription() + ":"
											+ parameters[0].toString() + "</td>\n");
						} else {
							sb2.append(
									"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_funtcion_td\">"
							...
							sb2.append(
									"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_name_td\">"
											+ passArrayList.get(i).getMethod().getDescription() + "</td>\n");
						}
					} else {
						sb2.append("<tr  style=\"color:green\"  id=\"" + testMethodFullName + "\">\n");
						sb2.append(
					...
					sb2.append(" ")
							.append("<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_result_td\"><label class=\"passed_label\" id=\"passedresult_"
									+ passArrayList.get(i).getMethod().getMethodName() + "\">"
									+ this.getStatus(passArrayList.get(i).getStatus()) + "</label><br /></td>" + "\n");

					if (passArrayList.get(i).getParameters().length > 0) {
						sb2.append(
								"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_result_beizhu_td\">"
										+ "输入数据：" + passArrayList.get(i).getParameters()[0] + "</td>\n");
					} else {
						sb2.append(
								"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_result_beizhu_td\">"
										+ "输入数据：无" + "</td>\n");
				...
				if (!skipArrayList.get(j).getMethod().getDescription().isEmpty()) {
					if (skipArrayList.get(j).getParameters().length > 0) {
						sb2.append("<tr style=\"color:gray\" id=\"" + testMethodFullName + "."
								+ skipArrayList.get(j).getEndMillis() + "\">\n");
						sb2.append(
								"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"SEQ\">"
										+ (j + failedArrayList.size() + passArrayList.size() + 1) + "</td>");
						Object[] parameters = skipArrayList.get(j).getParameters();
						if (parameters[0].toString().contains("院") || parameters[0].toString().contains("所")
								|| parameters[0].toString().contains("医") || parameters[0].toString().contains("病")
								|| parameters[0].toString().contains("治")) {
							...
							sb2.append(
									"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_name_td\">"
											+ skipArrayList.get(j).getMethod().getDescription() + ":"
											+ parameters[0].toString() + "</td>\n");
						} else {
							sb2.append(
									"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_funtcion_td\">"
							...
							sb2.append(
									"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_name_td\">"
											+ skipArrayList.get(j).getMethod().getDescription() + "</td>\n");
						}
						...
						sb2.append(
								"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"SEQ\">"
										+ (j + fullResults.size() + passArrayList.size() + 1) + "</td>");
						sb2.append(
								"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_funtcion_td\">"
										+ skipArrayList.get(j).getTestContext().getName() + "</td>\n");
						sb2.append(
					...
					if (skipArrayList.get(j).getParameters().length > 0) {
						sb2.append(
								"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_result_beizhu_td\">"
										+ "输入数据：" + skipArrayList.get(j).getParameters()[0] + "</td>\n");
					} else {
						sb2.append(
								"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_result_beizhu_td\">"
										+ "输入数据：无" + "</td>\n");
					...
					sb2.append(
							"<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" class=\"case_time_td\">"
									+ formateTime((long) t) + "</td>\n")
							.append("<td  style=\"border:1.5px;border-color:gray; word-break:break-all;border-style: solid;height:20px;\" colspan=\"3\" style=\"display:none\">"
									+ "" + "</td>\n")
							.append("</tr>\n");
				}
			}
